...
\begin{itemize}
  \item 
    We tested two additional settings of the softmax behavior selection
    greediness parameter, $\beta=0.01,1.0$, which reproduced qualitative patterns in
    the evolution of social learning. 
  \item
    We see that certain details are changed within this broad agreement between
    results obtained with different $\beta$. 
    \begin{enumerate}
      \item 
        More drift when $\beta=1$, which can be seen in certain $(\pilow, B)$ pairs
        (entries in plot matrices in Figure~\ref{fig:softmaxSensitivity}b). 
