...
co-evolved with the capacity for social learning, and we will show this has a marked influence 
on the cultural evolution of social learning.
...
learning, with all other uncertainty parameters held constant. This is because,
first, it is more difficuilt to find the optimal behavior with a larger $B$.
% more options present a more difficult problem for trial-and-error learning within
% a lifespan. 
Second, this effect is amplified because agents in a world with larger $B$ are more
...
sufficiently limited. Our model also demonstrated a new insight into social
learning as a scaffold for individual learning, enabled by endowing agents
with an evolutionarily- and biologically-plausible individual learning
mechanism, softmax search. Specifically, we found that agents could recover
from receiving outdated information, so social learning could evolve even
though social information was outdated more often than not. We found increased
payoff ambiguity and lifespan led to greater drift, which is essentially
evolutionary uncertainty in a finite-size population. Detailed theoretical
models and predictions of cultural evolution under uncertainty are critical
for understanding how humans adapt to an uncertain and rapidly changing world,
both to mitigate existential threats~\cite{Moya2020,Jones2021}, especially in
...
Model populations reached fixation between few to 140 generations on
...
Drift is greater for smaller populations sizes, especially when
$\pilow=0.8$, but patterns generally remain.
...
Slightly more drift when $N_T = 2$, slightly less drift when $N_T = 20$, 
but mostly just reproduces results for main text setting $N_T = 5$.
...
$\beta=1,100$, which reproduced qualitative patterns in the evolution of social learning. In
the main text we used $\beta=10$.  Softmax greediness is achieved formally by increasing the
...
explore less profitable alternatives.  If $\beta$ is too great the agents could more regularly
settle on a sub-optimal behavior because it paid off first.  In a fuller evolutionary
exploration of model parameters it would make sense for $\beta$ to co-evolve with the social
learner trait.  However, our sensitivity analysis demonstrates that our main conclusions of how
social learning evolves under uncertainty remain unchanged for different $\beta$ over three
orders of magnitude.

When $\beta=1$, most general trends are preserved, but in many cases the evolution
...
Other smaller details can also be observed between tested $\beta$. 
First, there is generally more drift when $\beta=1$, 
which can be seen in certain $(\pilow, B)$ pairs
(entries in plot matrices in Figure~\ref{fig:softmaxSensitivity}a). 
For example, when $\pilow = 0.1$ and $B=10$, there are a few more 
data points with greater drift, i.e., where $\meansl \approx 0.5$. 
The variation in our results when we set $\beta=100$
(Figure~\ref{fig:softmaxSensitivity}b), changed from the
main text where $\beta=10$, are more varied and subtle compared
to setting $\beta=1$. In some cases, such as $(\pilow=0.1, B=10)$ 
and $(\pilow=0.45, B=2)$,
