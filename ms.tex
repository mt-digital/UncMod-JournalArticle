...
The optimal behavior does not change within generations.
...
agents can explore alternatives and greedily perform a different observed optimal behavior
than an outdated optimal behavior they learned about socially. We rely
on this mechanism in the Analysis to explain the patterns we observe where
...
$u$. If a random draw dictates the behavior should change, the new
behavior is chosen from all other behaviors with the current optimal behavior
excluded.
...
for the same $u$, $B$, and $L$. When $\pilow \to \pihigh$, agents do better and better
